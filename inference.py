...
if args.checkpoint_path:
    if "reward" in args.checkpoint_path.lower():
        state_dict = torch.load(args.checkpoint_path, map_location="cpu")
        pipeline.generator.load_state_dict(state_dict)
        print(f"Loaded checkpoint from {args.checkpoint_path} without FSDP renaming")
    else:
        # FSDP training checkpoint (e.g. checkpoint_model_XXXXXX/model.pt)
        state_dict = torch.load(args.checkpoint_path, map_location="cpu")
        ...
        for name, param in generator_state_dict.items():
            renamed_state_dict[rename_param(name)] = param
        pipeline.generator.load_state_dict(renamed_state_dict, strict=False)
        print(f"Loaded checkpoint from {args.checkpoint_path} with FSDP renaming")
    ...
    if idx < num_prompts:
        ...
        for seed_idx in range(args.num_samples):
            ...
            if args.save_with_index:
                ...
                output_path = os.path.join(args.output_folder, f'{safe_prompt}-{file_idx}-{str(time.time())}.mp4')
            # Ensure the output directory exists
            os.makedirs(os.path.dirname(output_path), exist_ok=True)
            write_video(output_path, video[seed_idx], fps=16)
            print(f"Saved video to {output_path}")

    print(f"Single video inference time: {time.time() - single_video_start_time:.2f} seconds | {(time.time() - single_video_start_time)/60:.2f} minutes")

print(f"Total time taken: {time.time() - start_time:.2f} seconds | {(time.time() - start_time)/60:.2f} minutes")
