...
class CausalInferencePipeline(torch.nn.Module):
    ...
    def inference(
        ...
        # ------------------------------------------------------------ #
        # def kv_flush(scene_cut_needed, device):     # @hidir: original kv-flush
        ...
        def kv_flush(scene_cut_needed, device):     # @minar: modified kv-flush to keep more context for better long-range consistency, especially for large water bodies
            n_layers = len(self.crossattn_cache)
            for i in range(n_layers):
                self.crossattn_cache[i]['is_init'] = False
                # Keep frame 0 (anchor) + last 5 frames (enough flow context for large water)
                self.kv_cache1[i]['k'][:, 1560:9360] = self.kv_cache1[i]['k'][:, -7800:]
                self.kv_cache1[i]['v'][:, 1560:9360] = self.kv_cache1[i]['v'][:, -7800:]
                self.kv_cache1[i]['local_end_index'] = torch.tensor([9360], dtype=torch.long, device=device)
                self.kv_cache1[i]['scene_cut'] = scene_cut_needed
        # ------------------------------------------------------------ #
        for current_block_index, current_num_frames in enumerate(all_num_frames):
            ...
            # else:   # @minar: keep it natural, consistent flow stream  # rope-cut-loop
            #     ROPE_CUT_PERIOD = 12    # @minar smooth transition
            #     ROPE_CUT_WARMUP = 21
            #     is_rope_cut = (current_start_frame >= ROPE_CUT_WARMUP) and (current_start_frame % ROPE_CUT_PERIOD == 0)
            #     n_layers = len(self.crossattn_cache)
            #     for i in range(n_layers):
            #         self.kv_cache1[i]['scene_cut'] = is_rope_cut

            else:    # @minar - steady-forcing: flush periodically to stay in the initial-quality regime, even without explicit scene cuts
                FLUSH_PERIOD = 21   # flush every 21 blocks (~5s) to stay in initial-quality regime
                FLUSH_WARMUP = 21   # let first full window generate normally before flushing
                n_layers = len(self.crossattn_cache)
                if current_start_frame >= FLUSH_WARMUP and (current_block_index % FLUSH_PERIOD == 0):
                    kv_flush(False, noise.device)  # False = no scene_cut, smooth continuation
                else:
                    for i in range(n_layers):
                        self.kv_cache1[i]['scene_cut'] = False
            # ---------------------------------------------------------------- #
