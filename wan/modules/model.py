...
class WanRMSNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        # return self._norm(x).type_as(x) * self.weight    # @minar: for training with reduced precision - eliminate the float32 cast in RMSNorm
        return self._norm(x.float()).type_as(x) * self.weight    # inference
